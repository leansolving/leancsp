...
`Schur.schur_sb 5 _ 2` is the Schur CSP `schur_csp 5 2` (`{1,…,5}` 2-coloured with
colours `{0,1}`, no monochromatic `x + y = z`) **extended with the verified colour-fixing
symmetry-breaking constraint** `equals_const x₀ 0` (`Schur.schur_sbc`, proved a
...
`certs/schur_2_5_sbc.pbp`.  The *end-to-end* theorem `¬ (schur_csp 5 2).isSatisfiableInt`
...
def sb_schur_2_5 : IntCSP := Schur.schur_sb 5 (by decide) 2

/-- **Symmetry-broken `S(2) < 5`: `{1,…,5}` has no sum-free 2-colouring with `x₀` fixed.** -/
theorem sb_schur_2_5_unsat : ¬ sb_schur_2_5.isSatisfiableInt :=
  csp_unsat_file sb_schur_2_5 5 "certs/schur_2_5_sbc.pbp"

/-- The Schur `S(3) < 14` CSP extended with the colour-fixing SBC `x₀ = 0`. -/
def sb_schur_3_14 : IntCSP := Schur.schur_sb 14 (by decide) 3

/-- **Symmetry-broken `S(3) < 14`: `{1,…,14}` has no sum-free 3-colouring with `x₀` fixed.** -/
theorem sb_schur_3_14_unsat : ¬ sb_schur_3_14.isSatisfiableInt :=
  csp_unsat_file sb_schur_3_14 28 "certs/schur_3_14_sbc.pbp"
