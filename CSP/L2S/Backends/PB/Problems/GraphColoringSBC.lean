...
the verified colour-fixing SBC `equals_const x₀ 0` (`graph_coloring_sbc`, proved a domain
...
def sb_k3_2col : IntCSP := graph_coloring_sb 3 (by decide) k3_edges 2

/-- `K₄`/3-colouring extended with the colour-fixing SBC `x₀ = 0`. -/
def sb_k4_3col : IntCSP := graph_coloring_sb 4 (by decide) k4_edges 3

/-- **Symmetry-broken: `K₃` is not 2-colourable** (with `x₀` fixed). -/
theorem sb_k3_2col_unsat : ¬ sb_k3_2col.isSatisfiableInt :=
  csp_unsat_file sb_k3_2col 3 "certs/k3_sbc.pbp"

/-- **Symmetry-broken: `K₄` is not 3-colourable** (with `x₀` fixed). -/
theorem sb_k4_3col_unsat : ¬ sb_k4_3col.isSatisfiableInt :=
  csp_unsat_file sb_k4_3col 8 "certs/k4_sbc.pbp"
