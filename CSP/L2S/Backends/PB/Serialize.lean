...
Serializes an `Array Sat.PB.Constr` to the OPB text RoundingSat consumes.  This
is **outside the trust base**: a wrong serialization makes the external solver
prove the wrong formula, but the reflection checker then rejects the returned
certificate against the *Lean-side* constraint array, so a mismatch can only
cause a failure to elaborate, never an unsound theorem.

Format: 1-based vars `x{i+1}`, `~` for negation, `>=`, trailing ` ;`. -/
