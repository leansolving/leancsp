...
/-!
# PB backend — end-to-end UNSAT for a *real* `IntCSP` (PLAN.md M5)
...
honest `IntCSP` — three integer variables bounded to `{0,1,2,3}` with the
...
`¬ demoHomCSP.isSatisfiableInt` from the committed PB certificate
