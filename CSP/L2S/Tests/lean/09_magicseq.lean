...
def magic_bounds (n : ℕ) : List (TaggedConstraint n) :=
...
def magic_count_constraints (n : ℕ) : List (TaggedConstraint n) :=
  let all_vars : _root_.Vector (HomogeneousVarIndex n) n := _root_.Vector.ofFn id
  List.finRange n |>.map fun i =>
    count_var all_vars (i.val : ℤ) i

-- General parametric magic sequence CSP
def magic_sequence_csp (n : ℕ) : HomogeneousCSP :=
  ⟨n, magic_bounds n ++ magic_count_constraints n⟩

-- Specific instance: n=10
def magicseq10 : HomogeneousCSP := magic_sequence_csp 10
