...
/-- Schur CSP -/
def schur_csp_sb (n : ℕ) (colors : ℕ) (triples : List (Fin n × Fin n × Fin n)) : IntCSP :=
...
def schur_sb_constraint (n : ℕ) (h_n : 0 < n) : IntConstraint n :=
  equals_const ⟨0, h_n⟩ 0

/-- Extended CSP (including the SBC) -/
def extended_schur_csp (n : ℕ) (h_n : 0 < n) (colors : ℕ) (triples : List (Fin n × Fin n × Fin n)) : IntCSP :=
  (schur_csp_sb n colors triples).addConstraint (schur_sb_constraint n h_n)
...
theorem schur_color_swap_is_symmetry (n colors : ℕ)
    ...
    (h_colors : 0 < colors)
    (h_c : 0 ≤ c ∧ c < colors) :
    DomainSymmetry (schur_csp_sb n colors triples) (schur_color_swap c) := by
...
theorem schur_sb_is_domain_symmetry_breaking (n colors : ℕ)
    (h_n : 0 < n) (h_colors : 0 < colors)
    (triples : List (Fin n × Fin n × Fin n)) :
    domainSymmetryBreakingConstraint
      (schur_csp_sb n colors triples)
      (schur_sb_constraint n h_n) := by
  ...
  · -- Identity case: solution already satisfies the SBC
    ...
    · intro tc h_tc_mem
      ...
      · rw [h_sbc]
        simp only [IntCSP.satisfiesConstraintInt, schur_sb_constraint, equals_const, patternHolds,
          valAt, IntCSP.addConstraint, schur_csp_sb, h_n, dif_pos, Function.comp_apply,
  ...
  · -- Swap case: apply color swap to fix color of first integer
    ...
    have h_c_in_bounds : 0 ≤ c ∧ c < colors := by
      have h_bound : bound ⟨0, h_n⟩ 0 (colors - 1) ∈ (schur_csp_sb n colors triples).constraints := by
...
theorem schur_sb_is_symmetry_breaking (n colors : ℕ)
    (h_n : 0 < n) (h_colors : 0 < colors)
    (triples : List (Fin n × Fin n × Fin n)) :
    symmetryBreakingConstraint
      ...
      (schur_sb_constraint n h_n) := by
  left
  exact schur_sb_is_domain_symmetry_breaking n colors h_n h_colors triples
...
theorem schur_sb_equisatisfiability (n colors : ℕ)
    ...
    equisatisfiable
      (schur_csp_sb n colors triples)
      (extended_schur_csp n h_n colors triples) := by
...
def schur_sb (n colors : ℕ) : IntCSP :=
  schur_csp_sb n colors (schurTriples n)

/-- Extended Schur CSP with SBC, triples computed from n -/
def extended_schur (n : ℕ) (h_n : 0 < n) (colors : ℕ) : IntCSP :=
  extended_schur_csp n h_n colors (schurTriples n)

theorem schur_sb_equisatisfiability' (n colors : ℕ)
    (h_n : 0 < n) (h_colors : 0 < colors) :
    equisatisfiable (schur_sb n colors) (extended_schur n h_n colors) :=
