...
> ⚠️ **AXIOM WARNING — this module is NOT axiom-clean.**
>
> The headline theorems here (`twin_permutation_is_variable_symmetry`,
> `twin_ordering_is_variable_symmetry_breaking`, and everything downstream)
> depend on **two unproven `axiom`s**:
>   * `gate_constraint_preserved_by_twin_perm`
>   * `ordering_constraint_satisfied_by_sort`
> Verify with `#print axioms twin_ordering_is_variable_symmetry_breaking`.
>
> These axioms were inherited from the original (pre-refactor) development; they
> assert facts the authors believed sound but did not formalise. **Treat the
> results in this file as conjectures, not theorems.** This is the ONLY file in
> `CSP/L2S/Proofs/` that is not axiom-clean — every other proof depends on at most
> `propext, Classical.choice, Quot.sound`.
>
> **For future work: do not add new `axiom`s. Discharge these two (replace each
> `axiom` with a `theorem … := by …`) before relying on this module.** See the
> doc comments at each axiom for the proof obligations they stand in for.
...
theorem twin_permutation_is_variable_symmetry
  ...
  · -- Case 1: Bound constraint
    ...
    have h_βv_sat := h_sol (bound (β v) 0 1) h_βv_bound
