...
Reads experiments/sbc/results/sbc_scaling.csv and, per family, reports (geomean only — no
"greatest instance" columns):

  * wall-clock solver time without / with the SBC, and their per-instance speedup,
  * roundingsat deterministic time (machine-independent search effort), same three stats,
  * our pipeline's own PBLean checking cost without / with the SBC (`check_us`), and the
    fuller reflected-term verify wall (`verify_wall_s`).

Both wall and deterministic time are recorded so either can go in the paper.
